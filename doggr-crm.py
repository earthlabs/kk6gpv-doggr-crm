...
class CRMP(object):
    ...
    def __init__(self, tau_max=50, f_max=1, p_max=50, iter_max=200, tr_dist=1000, dist_wells=None, use_bhp=False):
    ...
    def init_params_sp(self):
        ...
        if self.dist_wells is not None:
            ...
            self.B1 = np.random.random(
                (self.num_p, self.num_i)) * (0.5 - 0.01) + 0.01
        ...
        if self.use_bhp:
            ...
            self.model_params = np.concatenate(
                (self.A.reshape(1, -1), self.B2.reshape(1, -1),
    ...
    def reconstr_sparse_B1(self, B1):
        ...
        for i in range(self.num_p):
            B1_sp[self.indp[i]:self.indp[i + 1]] = B1[i,
                                                      self.ind[self.indp[i]:self.indp[i + 1]]]
        return B1_sp
    ...
    def Qt_hat_dt_sp(self, params, X, well_on):
        ...
        Qt_pred = np.multiply((np.multiply(
            A, Qt.T) + np.multiply((np.ones_like(A) - A), np.dot(B, u))).T, well_on)
    ...
    def fit(self, X=None, y=None, use_fprime=False):
        ...
        if use_fprime:
            model_params, fx, iters, exit, _ = fmin_slsqp(func=self.f_to_opt_sp, x0=self.model_params, args=(X, y),
                                                          bounds=self.param_bounds_sp(),
                                                          fprime=self.d_f_to_opt_sp,
                                                          # f_eqcons = self.B_constr_eq,
                                                          f_ieqcons=self.B_constr_ineq_dt_sp,
                                                          iprint=0, iter=self.iter_max, full_output=True)
        else:
            model_params, fx, iters, exit, _ = fmin_slsqp(func=self.f_to_opt_sp, x0=self.model_params, args=(X, y),
                                                          bounds=self.param_bounds_sp(),
                                                          # fprime=self.d_f_to_opt_sp,
...
#         print('model fit is ok')
    ...
    def plot_results(self):
        ...
        for idx, prd in enumerate(wells['prds'].keys()):
            ...
            wells['prds'][prd]['fit'] = pd.Series(self.predict().T[idx],
        ...
        for idx, inj in enumerate(wells['injs'].keys()):
            ...
            for jdx, prd in enumerate(wells['prds'].keys()):
                ...
                if self.B1.T[idx][jdx] > 0.5:
                    ...
                    fig_mapb.add_trace(
                        go.Scattermapbox(
                            lon=[wells['injs'][inj]['x'],
                                 wells['prds'][prd]['x']],
                            lat=[wells['injs'][inj]['y'],
                                 wells['prds'][prd]['y']],
                            mode='lines',
                            line=dict(color=clr, width=self.B1.T[idx][jdx]*10),
                        )
                    )
    ...
    def save_cons(self, api):
        ...
        self.taus = []
        for idx, prd in enumerate(wells['prds'].keys()):
            self.taus.append(dict(prd=prd,
                                  tau=self.A[idx][0],
                                  fj_sum=self.B1[idx].sum()
                                  ))
            wells['prds'][prd]['tau'] = self.A[idx][0]
            wells['prds'][prd]['fj_sum'] = self.B1[idx].sum()
            wells['prds'][prd]['fit'] = pd.Series(self.predict().T[idx],
        ...
        for idx, inj in enumerate(wells['injs'].keys()):
            if inj == api:
                wells['injs'][inj]['fi_sum'] = self.B1.T[idx].sum()
                for jdx, prd in enumerate(wells['prds'].keys()):
                    self.cons.append(dict(frm=inj,
                                          to=prd,
                                          gain=self.B1.T[idx][jdx],
                                          x0=wells['injs'][inj]['x'],
                                          y0=wells['injs'][inj]['y'],
                                          xm=np.mean(
                                              [wells['injs'][inj]['x'], wells['prds'][prd]['x']]),
                                          ym=np.mean(
                                              [wells['injs'][inj]['y'], wells['prds'][prd]['y']]),
                                          x1=wells['prds'][prd]['x'],
                                          y1=wells['prds'][prd]['y'],
                                          ))
            wells['injs'][inj]['cons'] = self.cons
    ...
    def process_well(self, offsets):
        ...
        data_oil = pd.pivot_table(offsets, values='oil', index=[
                                  'date'], columns=['api'], aggfunc=np.sum)
        data_wtr = pd.pivot_table(offsets, values='water', index=[
                                  'date'], columns=['api'], aggfunc=np.sum)
        ...
        for idx, well in enumerate(data_prod.columns):
            prds[well] = dict(type='prd',
                              x=locs[locs['api'] == well[-8:]
                                     ]['longitude'].values[0],
                              y=locs[locs['api'] == well[-8:]
                                     ]['latitude'].values[0],
                              prod=data_prod[well]
                              )
        ...
        data_cyc = pd.pivot_table(offsets, values='cyclic', index=[
                                  'date'], columns=['api'], aggfunc=np.sum)
        data_stm = pd.pivot_table(offsets, values='steam', index=[
                                  'date'], columns=['api'], aggfunc=np.sum)
        data_wtri = pd.pivot_table(offsets, values='water_i', index=[
                                   'date'], columns=['api'], aggfunc=np.sum)

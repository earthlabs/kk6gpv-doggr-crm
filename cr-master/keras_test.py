...
def reshape_for_lstm(data, y, window_size=1):
    transformed_data = np.zeros(
        (data.shape[0] - window_size + 1, window_size, data.shape[1])
    )
...
x_train_trans, y_train_trans = reshape_for_lstm(
    x_train, y_train, window_size=window_size
)

x_test_trans, y_test_trans = reshape_for_lstm(
    x_test, y_test, window_size=window_size
)
#%%
y_test_trans.shape
#%%
def smae(y_target, y_predicted):
    return np.average(
        np.absolute(y_target - y_predicted)
        / (np.absolute(y_target + y_predicted) / 2),
        axis=0,
    )
...
Qt_pred = np.concatenate(
    (
        Qt_target[: 100 + window_size - 1, :],
        Qt_pred_train_step,
        Qt_target[300 : 300 + window_size - 1, :],
        Qt_pred_test_step,
    ),
    axis=0,
)
...
def predict_sequence_full(model, X, num_p, lstm=False, window_size=1):
    ...
    if lstm:
        for i in range(window_size, X.shape[0]):
            X[i, :num_p] = model.predict(
                reshape_for_lstm(X[i - window_size : i, :], None, window_size)
            )
    else:
...
#%%
y_pred_train_seq = predict_sequence_full(
    model_mse, x_train, 5, lstm=True, window_size=window_size
)
y_pred_test_seq = predict_sequence_full(
    model_mse, x_test_null, 5, lstm=True, window_size=window_size
)
Qt_pred_test_seq = X_scaler.inverse_transform(y_pred_test_seq)
Qt_pred_train_seq = X_scaler.inverse_transform(y_pred_train_seq)
Qt_pred_seq = np.concatenate(
    (Qt_target[:100, :], Qt_pred_train_seq, Qt_pred_test_seq), axis=0
)
#%%
...
g2.ax_joint.set(
    xlabel="История, Дебит жидкости, м3/сут",
    ylabel="Прогноз, Дебит жидкости, м3/сут",
